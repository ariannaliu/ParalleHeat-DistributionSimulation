...
namespace hdist {
    ...
    bool calculate(const State &state, Grid &grid, int local_size, int room_size, int rank) {
        bool stabilized = true;

        switch (state.algo) {
            case Algorithm::Jacobi:
                for (size_t i = (rank-1)*local_size; i < rank*local_size; ++i) {
                    for (size_t j = 0; j < state.room_size; ++j) {
                        if((i*grid.length+j)<(state.room_size*state.room_size)){
